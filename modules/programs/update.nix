{
  ...
  Update_script = pkgs.writeShellApplication {
    ...
    # Default commands
    NH_DRY_COMMAND='nh os switch -H "'$(hostname)'" -n'
    NH_BUILD_COMMAND='nh os switch -H "'$(hostname)'"'
    NH_UPGRADE_COMMAND='nh os switch -H "'$(hostname)'" -u'
    NIX_DRY_COMMAND='nixos-rebuild dry-build --flake .#'$(hostname)'''
    NIX_BUILD_COMMAND='nixos-rebuild switch --flake .#'$(hostname)'''
    NIX_UPGRADE_COMMAND='nix flake lock'
